...
def check_events(ai_settings, screen, stats, sb, play_button, ship, aliens, bullets):
    ...
    for event in pygame.event.get():       
        if event.type == pygame.QUIT:
            write_high_score("high_score.txt",stats)
            pygame.quit()
            sys.exit()
            ...
            check_keydown_events(event, ai_settings, screen, stats, sb, play_button, ship, aliens, bullets)     #调用函数check_keydown_events
...
def get_number_aliens_x(ai_settings,alien_width):         
    ...
    availiable_space_x = ai_settings.screen_width - 3 * alien_width   
...
def write_high_score(fill_name,stats):
    f = open(fill_name,"w")
